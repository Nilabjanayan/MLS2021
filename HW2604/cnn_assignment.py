...
classes_ = train.iloc[:,1].unique()
classes = {str(class_):index for index,class_ in enumerate(classes_)}
classes
...
class MyDataset():
  ...
  def __getitem__(self,idx,classes = classes):
    ...
    if self.argument:
      ...
      pilx = Image.fromarray(img)
      X = transform2(pilx)
    Y = classes[self.classlabels[idx]]
    return X,Y
...
class CNN(nn.Module):
    def __init__(self,n_hidden,n_output):
        ...
        self.fc1 = nn.Linear(64 * 4 * 4, n_hidden)
        # linear layer (500 -> 10)
...
optimizer = optim.SGD(model.parameters(), lr=0.02)   #solver
...
train_lss, valid_lss = Model_train(30 , model = model, criterion = criterion, optimizer = optimizer)
...
for i in range(10):
    if class_total[i] > 0:
        print('Test Accuracy of %5s: %2d%% (%2d/%2d)' % (
            list(classes.keys())[i], 100 * class_correct[i] / class_total[i],
